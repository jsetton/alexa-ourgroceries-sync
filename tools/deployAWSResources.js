 ...
 * @type {String}
 */
const LAMBDA_CODE_PATH = '../lambda/householdList';
 ...
 * @type {String}
 */
const FUNCTION_NAME = 'alexa-ourgroceries-sync';

/**
 * IAM policy name
 * @type {String}
 */
const POLICY_NAME = 'AlexaOurGroceriesSync';

/**
 * IAM role name
 * @type {String}
 */
const ROLE_NAME = `ask-lambda-OurGroceriesSync`;
...
async function deployLambdaFunction(roleArn) {
  ...
  // Create/update lambda function code
  const results = await executeAWSCommand(
    `lambda get-function --function-name ${FUNCTION_NAME}`
  ).then(() => Promise.all([executeAWSCommand(
    `lambda update-function-code --function-name ${FUNCTION_NAME} --zip-file fileb://${zipFile}`
  ), executeAWSCommand(
    `lambda update-function-configuration --function-name ${FUNCTION_NAME} --cli-input-json file://${lambdaConfigFile}`
  )])).catch(() => Promise.all([executeAWSCommand(
  ...
  // Add alexa skill kit invoke permission if no policy defined
  await executeAWSCommand(
    `lambda get-policy --function-name ${FUNCTION_NAME}`
  ).catch(() => executeAWSCommand(
    `lambda add-permission --function-name ${FUNCTION_NAME} --action lambda:InvokeFunction --principal alexa-appkit.amazon.com --statement-id alexa-skill`
  ));
  // Return function arn
  return results[0].FunctionArn;
}
