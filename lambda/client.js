...
export default class SyncListClient {
  ...
  async updateAlexaList() {
    const [alexaList, ourGroceriesList] = await Promise.all([
      this.getAlexaShoppingList(),
      this.getOurGroceriesShoppingList()
    ]);
    const promises = [];
    ...
    ourGroceriesList.items.forEach((ourGroceriesItem) => {
      ...
      const alexaItem = alexaList.items.find(
        (alexaItem) => alexaItem.value.toLowerCase() === ourGroceriesItem.value.toLowerCase()
      );
      // Determine alexa status based of ourGroceries crossed off property
      const ourGroceriesStatus = !ourGroceriesItem.crossedOffAt ? 'active' : 'completed';
      ...
      if (typeof alexaItem !== 'undefined') {
        ...
        promises.push(
          ourGroceriesStatus === alexaItem.status
            ? getItemProperties(alexaItem)
            : this.householdListManager
                .updateListItem(alexaList.listId, alexaItem.id, {
                  value: alexaItem.value,
        ...
        promises.push(
          this.householdListManager
            .createListItem(alexaList.listId, {
              value: ourGroceriesItem.value,
              status: ourGroceriesStatus
            })
            .then((item) => getItemProperties(item))
        );
    ...
    alexaList.items
      .filter((alexaItem) =>
        ourGroceriesList.items.every(
          (ourGroceriesItem) => ourGroceriesItem.value.toLowerCase() !== alexaItem.value.toLowerCase()
        )
      )
      .forEach((alexaItem) => promises.push(this.householdListManager.deleteListItem(alexaList.listId, alexaItem.id)));
